...
object KafkaMutationGenericAvroWithDateProducer {
  def apply(config: Config) = new KafkaMutationGenericAvroWithDateProducer(config)
}
...
/** An implementation of the base KafkaMutationAvroProducer class that uses a
  ...
  */
class KafkaMutationGenericAvroWithDateProducer(config: Config)
